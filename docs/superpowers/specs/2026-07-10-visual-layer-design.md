...
**Eval-бар: цвет оценки — это поведение, а не оформление.** Число печатается у
нижнего края бара. Под ним либо светлая заливка белых, либо тёмный корпус бара —
смотря какова ориентация доски и доля белых. Константный цвет текста здесь
неверен: при мате против стороны, которой развёрнута доска, заливка нулевая, и
тёмный текст даёт на корпусе контраст 1.10:1. Цвет обязан выбираться. Это
единственное место, где визуальный слой добавляет логику, и потому единственное,
где к существующему тестовому файлу дописываются тесты.
